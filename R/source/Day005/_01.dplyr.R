...
## 정렬 : arrange(df, v) - 기본적으로 오름차순 or arrange(df, desc(v))
  arrange(mtcars, wt)
...
## 연비(mpg)를 이용하여 순위를 구하고 해당 순위를 파생변수로 mpg_rank 추가
  mutate(mtcars, mpg_rank = rank(mpg)) -> mtcars_rank
  head(mutate(mtcars, mpg_rank = rank(mpg)))
  
  arrange(mtcars_rank, mpg_rank)
