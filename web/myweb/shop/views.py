...
def product_list(request):
    ## 상품 테이블에 존재하는 모든 상품을 읽어와 웹에 뿌려주는 역할
...
def product_insert(request):
    ## 상품 등록폼을 통해 입력된 값을 가져와 db에 저장하는 작업을 수행하는 메서드
    ## 상품 관련 이미지가 존재하는지의 여부에 따라 처리가 달라진다.
    if "file1" in request.FILES:  # 상품 이미지가 존재한다면
    ...
    row = Product(product_name=request.POST['product_name'], price=request.POST['price'],
                  description=request.POST['description'], picture_url=file_name)
    row.save()
...
def product_update(request):
    ...
    row_src = Product.objects.get(product_id=id)  # 원본 데이터
    p_url = row_src.picture_url

    if 'file1' in request.FILES:  # = file1이 수정되었다면
        file = request.FILES['file1']
        p_url = file._name
...
def cart_list(request):
    ...
    if uid != "":
        ...
        if cartCount > 0:
            ...
            # 구매 금액이 50000원 이상이면 배송료 0, 그렇지 않으면 2500
            if sumMoney >= 50000:
                fee = 0
            else:
                fee = 2500
