...
def home(request):
    ...
    items = Address.objects.order_by('-idx')  # order_by('idx') = 오름차순
...
def insert(request):
    ...
    # sql = insert address into values(name, tell, email, address)
    addr.save()
    return redirect('/address')  # address의 url을 찾는 내용

def detail(request):
    # 주소록 목록에서 이름을 클릭하면 해당 이름의 사용자 정보를 상세보기 폼으로 보여주는 작업
    id = request.GET['idx']

    # db에서 해당 idx 객체를 가져오는 작업
    # sql = select * from address where idx = id
    # select * from address => Address.objects.all(조건 = where)
